...
def _get_image_blob(im):
    ...
    if im_size_max == im_size_fx:
        im_scale_fx = float(cfg.TEST.MAX_SIZE) / float(im_size_fx)
        im_scale_fy = float(cfg.TEST.MIN_SIZE) / float(im_size_fy)
    if im_size_max == im_size_fy:
        im_scale_fx = float(cfg.TEST.MIN_SIZE) / float(im_size_fx)
        im_scale_fy = float(cfg.TEST.MAX_SIZE) / float(im_size_fy)
    ...
    for target_size in cfg.TEST.SCALES:
        ...
        im = cv2.resize(im_orig, None, None, fx=im_scale_fx, fy=im_scale_fy,
...
def test_net(net, imdb, max_per_image=400, thresh=-np.inf, vis=False):
    ...
    #if cfg.TEST.model_type == "resnet":
    #    model_path = cfg.TEST.model_type + '_' + cfg.TEST.model_name + '_' + cfg.TEST.iter_num
