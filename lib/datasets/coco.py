...
class coco(imdb):
    ...
    def _write_coco_results_file_val(self, all_boxes, res_file):
        # [{"image_id": 42,
        #   "category_id": 18,
    ...
    def _write_coco_results_file_test(self, all_boxes, res_file1,res_file2,res_file3):
        # [{"image_id": 42,
        #   "category_id": 18,
        #   "bbox": [258.15,41.29,348.26,243.78],
        #   "score": 0.236}, ...]
        ...
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            if cls_ind>=0 and cls_ind<27:
                print 'Collecting {} results ({:d}/{:d})'.format(cls, cls_ind,
                                                          self.num_classes - 1)
                coco_cat_id = self._class_to_coco_cat_id[cls]
                results1.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
                print 'Writing results json to {}'.format(res_file1)
                with open(res_file1, 'w') as fid:
                     json.dump(results1, fid)
            if cls_ind>=27 and cls_ind<54:
                print 'Collecting {} results ({:d}/{:d})'.format(cls, cls_ind,
                                                          self.num_classes - 1)
                coco_cat_id = self._class_to_coco_cat_id[cls]
                results2.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
                print 'Writing results json to {}'.format(res_file2)
                with open(res_file2, 'w') as fid:
                     json.dump(results2, fid)
            if cls_ind>=54:
                print 'Collecting {} results ({:d}/{:d})'.format(cls, cls_ind,
                                                          self.num_classes - 1)
                coco_cat_id = self._class_to_coco_cat_id[cls]
                results3.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
                print 'Writing results json to {}'.format(res_file3)
                with open(res_file3, 'w') as fid:
                     json.dump(results3, fid)
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        if self._image_set.find('test') == -1:
            res_file = osp.join(output_dir, ('detections_' +
                                         ...
                                         '_results'))
            res_file += '.json'
            self._write_coco_results_file_val(all_boxes, res_file)
        else:
            print 'Evaluate {}_set'.format(self._image_set)
            res_file_1 = osp.join(output_dir, ('detections_' +
                                         self._image_set +
                                         self._year +
                                         '_results_1'))
            res_file_1 += '.json'
            res_file_2 = osp.join(output_dir, ('detections_' +
                                          self._image_set +
                                          self._year +
                                          '_results_2'))
            res_file_2 += '.json'
            res_file_3 = osp.join(output_dir, ('detections_' +
                                          self._image_set +
                                          self._year +
                                          '_results_3'))
            res_file_3 += '.json'
            self._write_coco_results_file_test(all_boxes, res_file_1,res_file_2,res_file_3)
